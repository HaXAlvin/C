...
//程式主要功能分為四個區塊，分別為1.新增/修改角色，2.顯示角色資料，3.選角以及4.戰鬥;
...
int main()
{
    int count_char = 3;                                  //用來計算角色數量的變數
    char name[5][21] = { " ","RTRT","DK","Hue"," " };    //角色名稱，name[1]、name[2]...分別對應到角色1、2、3、4的名稱;
    char type[5] = { ' ','F','G','W',' ' };              //角色屬性，排序同上;
    int  hp[5]   = { 0,45,40,45,0 };                     //角色生命值，第一個hp[0]是空的，為了讓陣列編號能對應到角色編號;
    int  atk[5]  = { 0,20,20,15,0 };                     //角色攻擊力，排序同上;
    int  def[5]  = { 0,5,10,10,0 };                      //角色防禦力，排序同上;
    ...
    while (1)
    {
        ...
        else if (choose == 1)        //輸入1，進入新增/修改角色迴圈;
        {
            //傳址分別為角色數量、第四位角色的名稱(字元陣列)、血量、攻擊、防禦、強度預測;
            EditNewChar(&count_char, name[4], &type[4], &hp[4], &atk[4], &def[4], &combat[4]);
        }
        else if (choose == 2)        //輸入2，顯示所有角色資料;
        {
            //傳值和傳址為所有角色的資料;
            PrintCharData(count_char, name, type, hp, atk, def, combat);
        }
        else if (choose == 3)        //輸入3，進入選角/戰鬥迴圈;
        {
            //大部分同上
...
void EditNewChar(int *counting, char new_name[], char *new_type, int *new_hp, int *new_atk, int *new_def, float *new_combat)        //新增/修改角色;
{
    while (1)
    {
        ...
        if (*counting == 4)           //若角色已有4名，則修改新增角色，否則新增角色;
            printf("修改新增角色\n");
        else if (*counting == 3)
            printf("新增角色\n");
        ...
        if (islower(*new_type))    //小寫轉大寫;
            *new_type = toupper(*new_type);

        type_check = create_type(*new_type);                             //若屬性不合規範，回傳1;
        ability_range = create_ability(*new_hp, *new_atk, *new_def);     //若能力值不合規範，回傳1;
        ...
        else        //輸入正確，印出角色四的數值，並將強度預測存入Array;
        {
            ...
            *new_combat = (*new_hp * 1.0 + *new_atk * 0.8 + *new_def * 0.5 - 50) * 6.5;
            ...
            if (*counting == 3)        //若目前只有三位角色，將紀錄角色數量的值改成四位;
                *counting = 4;
...
void PrintCharData(int counting_p, char show_name[][21], char show_type[], int show_hp[], int show_atk[], int show_def[], float show_combat[])     //顯示角色資料;
{
    for (int i = 1; i <= counting_p; i++)        //counting_p代表目前角色的數量;
    {
        printf("角色%d\n"
            "名稱：%s\n"
            "屬性：%c     攻擊：%d\n"
...
void SelectAndBattle(int counting_s, char select_type[], int select_hp[], int select_atk[], int select_def[])    //選角;
{
    while (1)
    {
        ...
        printf("選擇先攻角色：");
        scanf("%d", &first);
        member_check = member_range(first, counting_s);        //若選取角色不存在，回傳1;

        if (member_check)        //選取角色不存在，重置選角/戰鬥迴圈;
        {
            printf("錯誤：無此角色\n");
            continue;
        }

        int first_member[4] = { first,select_hp[first],select_atk[first],select_def[first] };        //角色存在，將角色的各項數據存入先攻成員的Array，以便戰鬥取用;
        char first_type = select_type[first];    //存取屬性;
        ...
        while (1)    //後攻選角迴圈;
        {
            ...
            printf("選擇後攻角色：");
            scanf("%d", &last);
            member_check = member_range(last, counting_s);     //同上，若選取角色不存在，回傳1;
        ...
        if (last == first)            //重複選角的情況，重置整個選角\戰鬥迴圈;
        {
            printf("錯誤：不可重複選取\n");
            continue;
        }

        int last_member[4] = { last,select_hp[last],select_atk[last],select_def[last] };        //角色存在，將角色的各項數據存入後攻成員的Array，以便戰鬥取用;
        char last_type = select_type[last];    //存取屬性;

        printf("\n先攻為角色%d，後攻為角色%d\n", first, last);

        int weakness = type_weakness(first_type, last_type);      //判定屬性剋制情況，根據情況不同會回傳1(先攻方克制後攻方)，2(後攻方克制先攻方)，0(並無相剋);
        Battling(first_member, last_member, weakness);            //進行戰鬥，傳送先後攻方的數值以及屬性相剋的情形;
        break;      //完成戰鬥，離開戰鬥迴圈;
    }
}

void Battling(int first[], int last[], int weakness_result)    //戰鬥迴圈;
{
    printf("戰鬥開始\n\n");
