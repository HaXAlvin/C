...
int role = 3;//一開始的角色數目
void PrintCharDate(char name[],char type,int atk,int hp,int def){
    printf("名稱:%s\n屬性:%c\t\t攻擊:%d\n血量:%d\t\t防禦:%d\n角色強度預測:%.2f\n\n",name,type,atk,hp,def,(hp * 1.0 + atk * 0.8 + def * 0.5 - 50.0) * 6.5);
    return;
}
void EditNewChar(char name[],char type[],int atk[],int hp[],int def[]){
    int n = 1;//flag
    float val[4] = {.0};
    do {
        n = 0;
    ...
    role = 4;
    printf("\n角色4\n");
    PrintCharDate(name,type[3],atk[3],hp[3],def[3]);
    return;
}
void Battling(int first_atk,int second_atk,char type[],int atk[],int hp[],int def[],float T_ratio[],float B_ratio){
...
void SelectAndBattle(int first_atk,int second_atk,int role,char type[],int atk[],int hp[],int def[],float T_ratio[],float B_ratio){
    int n = 1;
    do {
        do{
            n = 0;
    ...
    printf("\n戰鬥開始");
    Battling(first_atk,second_atk,type,atk,hp,def,T_ratio,B_ratio);
    return;
}
int main(){
    ...
    while (n == 1){
        ...
        switch (num){
            case 1:
                EditNewChar(name[3],type,atk,hp,def);
                n = 1;
                break;
            case 2:
                for (i = 0 ; i < role ; i++ ){
                    printf("角色%d\n",i+1);
                    PrintCharDate(name[i],type[i],atk[i],hp[i],def[i]);
                }
                n = 1;
                break;
            case 3:
                SelectAndBattle(first_atk,second_atk,role,type,atk,hp,def,T_ratio,B_ratio);
                n = 1;
                break;
            case 0:
                printf("結束遊戲\n");
