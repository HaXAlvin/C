...
int EditNewChar(int NewChar, char name[][10], char CharType[], int CharHp[], int CharAtk[], int CharDef[])
{
    ...
    do{//用do while迴圈 先輸入角色資訊後判斷是否輸入錯誤
        ...
        printf("防禦 : ");
        scanf("%d", &CharDef[3]);
        int sum = CharAtk[3] + CharHp[3] + CharDef[3];
        if (CharType[3] == 102 || CharType[3] == 119 || CharType[3] == 103 || CharType[3] == 97)//用ASCII code 將小寫轉成大寫
            CharType[3] = CharType[3] - 32;
        //先判斷屬性
        if (CharType[3] != 'F' && CharType[3] != 'W' && CharType[3] != 'G' && CharType[3] != 'A')
        {
            printf("錯誤:無此屬性\n");
            error_type = 1;
            //判斷屬性錯誤後接著判斷能力數值與其總和
            if (CharAtk[3] < 15 || CharAtk[3] > 20 || CharHp[3] < 40 || CharHp[3] > 50 || CharDef[3] < 5 || CharDef[3] > 10)
            {
        ...
        else
        {
            ...
            float predict = (CharHp[3] * 1.0 + CharAtk[3] * 0.8 + CharDef[3] * 0.5 - 50) * 6.5;

            printf("\n角色4\n名稱: %s\n屬性: %c      攻擊: %d\n", name[3], CharType[3], CharAtk[3]);
            printf("血量: %d     防禦: %d\n", CharHp[3], CharDef[3]);
            printf("強度預測: %.2f\n\n", predict);
            NewChar = 1;
        }
...
void PrintCharData(char name[], char type, int hp, int atk, int def)
{
    printf("\n名稱: %s\n屬性: %c      攻擊: %d\n", name, type, atk);
    printf("血量: %d     防禦: %d\n", hp, def);
    printf("強度預測: %.2f\n", (hp * 1.0 + atk * 0.8 + def * 0.5 - 50) * 6.5);
    return;
}

void SelectAndBattle(int NewChar, char type[], int hp[], int atk[], int def[])
{
    ...
    do{
        ...
        if (FirstAtk == SecondAtk)//重複選取同一個角色
        {
            printf("錯誤:不可重複選取\n");
            error = 1;//角色重複選取
        }
    ...
    printf("\n先攻為角色%d、後攻為角色%d\n", FirstAtk, SecondAtk);
    printf("戰鬥開始\n");
    Battling(FirstAtk, SecondAtk, type, hp, atk, def);
    return;
}

void Battling(int FirstAtk, int SecondAtk, char type[], int hp[], int atk[], int def[])
{
     //先攻角色資料
    char FirstAtk_type;
...
int main()
{
    ...
    while (printf("1. 新增或修改新增角色\n2. 顯示所有角色資料\n3. 戰鬥\n0. 結束遊戲\n請選擇: "), scanf("%d", &choice))
    {
        ...
        {
            NewChar = EditNewChar(NewChar, names, type, hp, atk, def);
        }
        else if (choice == 2)
        {
            for (int i = 0; i < NewChar + 3; i++)
            {
                printf("\n角色%d", i + 1);
                PrintCharData(names[i], type[i], hp[i], atk[i], def[i]);
            }
            printf("\n");
        }
        else if (choice == 3)
        {
            SelectAndBattle(NewChar, type, hp, atk, def);
        }
    }
    return 0;
}
