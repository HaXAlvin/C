...
int main()
{
    ...
    while(1){
        ...
        else if(select == 2)
        {
            //角色1數值
            printf("\n角色1\n名稱:%s\n",char_name[0]);
            printf("屬性:%c     攻擊:%d\n", char_type[0], char_atk[0]);
            printf("血量:%d    防禦:%d\n", char_hp[0], char_def[0]);
            printf("角色強度預測：%.2f\n\n",
                   (char_hp[0] * 1.0 + char_atk[0] * 0.8 + char_def[0] * 0.5 - 50.0) * 6.5 );
            //角色2數值
            printf("角色2\n名稱:%s\n",char_name[1]);
            printf("屬性:%c     攻擊:%d\n", char_type[1], char_atk[1]);
            printf("血量:%d    防禦:%d\n", char_hp[1], char_def[1]);
            printf("角色強度預測：%.2f\n\n",
                   (char_hp[1] * 1.0 + char_atk[1] * 0.8 + char_def[1] * 0.5 - 50.0) * 6.5 );
            //角色3數值
            printf("角色3\n名稱:%s\n",char_name[2]);
            printf("屬性:%c     攻擊:%d\n", char_type[2], char_atk[2]);
            printf("血量:%d    防禦:%d\n", char_hp[2], char_def[2]);
            printf("角色強度預測:%.2f\n\n",
                   (char_hp[2] * 1.0 + char_atk[2] * 0.8 + char_def[2] * 0.5 - 50.0) * 6.5 );
            getchar();
            if(is_creat == 1){
                printf("角色4\n名稱:%s\n",char_name[3]);
                printf("屬性:%c     攻擊:%d\n", char_type[3], char_atk[3]);
        ...
        else if(select == 3) //戰鬥
        {
            ...
            while(1)    //重複角色輸入的迴圈
            {
                while(1)
                {
                    ...
                    printf("\n選擇先攻角色:");
                    scanf("%d",&player1);
                    if(player1 < 1 || player1 > 3 + is_creat) //quest為有無角色4時數值是否增加
                    {
                        printf("錯誤:無此角色\n");
                        is_error2=1;
                    }
